...
4. 接続中は黄色で「Wi-Fi: Connecting to [SSID名]」と表示され、接続試行中のSSID名が表示されます。未接続時はグレーで表示されます
...
- **詳細情報**（小さく表示）:
  ...
  - Wi-Fi接続状態: 接続成功時は「Wi-Fi: [SSID]」（緑色）、接続中は「Wi-Fi: Connecting to [SSID名]」（黄色、接続試行中のSSID名を表示）、未接続時は「Wi-Fi: Offline」（グレー）
...
- WiFiネットワークが利用可能か確認
- 接続中はステータス表示が黄色で「Wi-Fi: Connecting to [SSID名]」と表示され、接続試行中のSSID名が確認できます
