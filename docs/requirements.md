...
- F10-3: Wi-Fi 接続に成功した場合、接続できた SSID をステータス表示に反映し、Wi-Fi 接続状態の表示を緑色にする。接続中は黄色で「Wi-Fi: Connecting to [SSID名]」と表示し、接続試行中のSSID名を表示する。未接続時はグレーで表示する。
...
- **Wi-Fi 接続状態**:
  - 接続成功時: 「Wi-Fi: [SSID]」（緑色）
  - 接続中: 「Wi-Fi: Connecting to [SSID名]」（黄色）。接続試行中のSSID名を表示する。
